...
class LanguageServiceImplTest {
	...
	@BeforeEach
	void preparation() {
		dao = mock(LanguageRepository.class);
		srv = new LanguageServiceImpl(dao);
	}
	...
	class Modify{
		...
		class OK{
			...
			void test1(){
				var test = mock(Language.class);
				when(test.isValid()).thenReturn(true);
				when(test.getLanguageId()).thenReturn(1);
				when(dao.existsById(anyInt())).thenReturn(true);
					try {
						srv.modify(test);
		...
		class KO{
			...
			void test4() {
				var test = mock(Language.class);
				when(test.isInvalid()).thenReturn(false);
				when(test.getLanguageId()).thenReturn(0);
				assertThrows(NotFoundException.class, () -> srv.modify(test));
			}
    ...
    class GetOne {
        ...
        void test1() {
            var expected = Optional.of(new Language(1));
    ...
    class GetAll {
        ...
        void test1() {
            var expected = List.of(new Language(1), new Language(2));
        ...
        void test3() {
            Page<Language> expected = mock(Page.class);
